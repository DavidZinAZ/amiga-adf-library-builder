"""Regression tests for the REAL Playmatch fetch path (issue #11 remediation).

These exercise the actual outbound fetch plumbing in ``playmatch.py`` that the
...
SENTINEL_HOST = "realplaymatch.test"
...
def _real_provider(base_url: str, cache: Path):
    ...
    cfg = pm.PlaymatchConfig.from_dict({"enabled": True, "base_url": base_url})
    assert cfg.enabled is True
    prov = pm.PlaymatchProvider(cfg, cache)  # no opener -> real fetch, resolve=True
    prov.discover()
    return prov
...
def test_opener_has_no_redirect_handler_and_clears_proxy():
    ...
    Proxy bypass (Defect 3) is exercised end-to-end by
    ``test_real_fetch_ignores_ambient_proxy`` (the hardened opener routes
    directly to the endpoint; the fake proxy IP is never a connect target).
    Here we assert the opener's handler chain includes our no-follow subclass
    and excludes the stdlib auto-following ``HTTPRedirectHandler``.
    """
    op = pm._build_no_redirect_opener(timeout=1.0)
    # Collect every handler instance the opener registered across its
    # request/response/open/error registries.
    instances = []
    ...
    # Our no-redirect subclass is present; the stdlib auto-follow handler is not.
...
def test_real_fetch_no_redirect_to_private_host(real_fetch_env, tmp_path):
    ...
    env = real_fetch_env
    # Server A redirects to the cloud-metadata link-local address on server B's
    # port. Server B counts requests; it must never be hit.
    ...
    # Redirect -> non-fatal miss (status>=300 refused by the real opener).
...
def test_real_fetch_success_within_bound(real_fetch_env, tmp_path):
    ...
    env = real_fetch_env
    env["handler_a"].body = (
        b'{"found": true, "provider_id": "PM-REAL", "title": "Real Game"}'
    )
    env["handler_a"].status = 200
    ...
    assert res.provider_id == "PM-REAL"


# --- Defect 3 (MEDIUM): ambient proxy ignored -------------------------------

def test_real_fetch_ignores_ambient_proxy(real_fetch_env, monkeypatch, tmp_path):
    """HTTP_PROXY/HTTPS_PROXY must not route the Playmatch request."""
    env = real_fetch_env
    env["handler_a"].body = b'{"found": true, "provider_id": "PM-NOPROXY"}'
    env["handler_a"].status = 200

    # Point the ambient proxy at a fake external host; the hardened opener must
    # ignore it and connect directly to the (translated) loopback endpoint.
    ...
    # Resolved correctly -> proves the proxy was bypassed (direct connect).
    assert res.found is True
    assert res.provider_id == "PM-NOPROXY"
