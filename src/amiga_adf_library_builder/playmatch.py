"""Optional Playmatch ROM-hash identity resolver + provider-ID capture.

This is an OPTIONAL provider. It is DISABLED by default and only activates when
an explicit ``[playmatch]`` TOML table is present AND ``enabled = true``.

Design posture (mirrors the local-media provider's security model, adapted for
a network correlation service):
...
* **Hash-first identity.** A precomputed public ``sha256`` (computed elsewhere,
  ...
  OUTRANKS any title/filename fallback. When Playmatch returns a provider-ID for
  the matched hash, it is captured in ``result.provider_id`` for downstream
  providers.
...
* **Privacy.** Only the public ``sha256`` (one-way, assumed public by the issue)
  and the canonical title may be sent to Playmatch. The cache stores ONLY
  ``provider_id`` + canonical title + a negative-lookup marker, keyed by the
  public signal. No private filename, path, or private hash is ever written to a
  cache or transmitted.

The synthetic request contract (documented here; a real server may differ):

* ``GET {base_url}/rom/{sha256}`` -> ``{"found": true, "provider_id": "<id>",
  "category": "<optional>", "title": "<canonical>", "confidence": <0..1>}``
  or ``{"found": false}``.
...
DEFAULT_BASE_URL = "https://api.playmatch.example/v1"
...
class PlaymatchMatchMethod(str, Enum):
    """How a Playmatch identity was resolved for a release group."""
...
class PlaymatchError(Exception):
    """Base error for Playmatch provider failures."""


class PlaymatchDisabled(PlaymatchError):
...
class PlaymatchConfig:
    """Typed view of the ``[playmatch]`` TOML table."""
    ...
    @classmethod
    def from_dict(cls, data: Optional[dict]) -> "PlaymatchConfig":
        """Build a config from the raw ``[playmatch]`` table (or None).
...
class PlaymatchResult:
    """Outcome of resolving Playmatch identity for one release group."""

    group_title: Optional[str]
    group_release_key: str
    found: bool = False
    category: Optional[str] = None
    match_method: PlaymatchMatchMethod = PlaymatchMatchMethod.NONE
...
@dataclass
class _Candidate:
    """One synthetic Playmatch match candidate (internal)."""
...
def _cache_file(cache_dir: Path, key: str) -> Path:
    ...
    return Path(cache_dir) / f"playmatch-{safe}.json"


def _cache_store(cache_dir: Path, key: str, entry: dict) -> None:
    """Write a privacy-bounded cache entry (provider_id + title + marker)."""
    try:
...
def _bounded_read(opener: Callable[..., bytes], url: str, *, timeout: float,
    ...
    if len(data) > max_bytes:
        raise PlaymatchError(
            f"Playmatch response exceeded max_response_bytes ({len(data)} > {max_bytes})"
        )
    return data
...
def _json_get(opener: Callable[..., bytes], url: str, *, timeout: float,
    ...
    Raises :class:`PlaymatchError` ONLY for programmer errors (a malformed URL
...
class PlaymatchProvider:
    ...
    Usage::
        ...
        cfg = load_playmatch_config(config_path)
        pm_cfg = PlaymatchConfig.from_dict(cfg)
        if pm_cfg.enabled:
            provider = PlaymatchProvider(pm_cfg, cache_dir)
    ...
    def resolve(self, group, *, scans: Optional[dict] = None,
                sha256: Optional[str] = None) -> PlaymatchResult:
        """Resolve Playmatch identity for ``group``.
        ...
        # Nothing found.
        result.match_method = PlaymatchMatchMethod.NONE
    ...
    def _resolve_by_hash(self, sha256: str, *, title: Optional[str],
                         release_key: str) -> Optional[PlaymatchResult]:
        ...
        if cached is not None:
            if cached.get("negative"):
                ...
                return PlaymatchResult(
                    ...
                    group_release_key=release_key,
                    found=False,
                    match_method=PlaymatchMatchMethod.NONE,
                    confidence=0.0,
            ...
            if pid:
                ...
                return self._exact_hash_result(
                    ...
                    method=PlaymatchMatchMethod.CANONICAL_REUSE,
                    release_key=release_key,
        ...
        if not provider_id:
            ...
            return PlaymatchResult(
                ...
                manual_review_reason="playmatch hash match returned no provider_id",
                match_method=PlaymatchMatchMethod.MANUAL_REVIEW,
        ...
        return self._exact_hash_result(
            ...
            category=category, confidence=EXACT_HASH_CONFIDENCE,
            method=PlaymatchMatchMethod.EXACT_HASH, release_key=release_key,
            provenance={"kind": "hash_match", "sha256": sha256[:8] + "..."},
        )

    def _exact_hash_result(self, *, sha256: str, provider_id: str, title: Optional[str],
                           category: Optional[str], confidence: float,
                           method: PlaymatchMatchMethod, release_key: str,
                           provenance: dict) -> PlaymatchResult:
        return PlaymatchResult(
            group_title=title,
            group_release_key=release_key,
            found=True,
    ...
    def _resolve_by_title(self, title: str, *, group, release_key: str
        ...
        if cached is not None and cached.get("provider_id"):
            return PlaymatchResult(
                ...
                found=True,
                category=cached.get("category"),
                match_method=PlaymatchMatchMethod.CANONICAL_REUSE,
                confidence=CANONICAL_REUSE_CONFIDENCE,
        ...
        if len(distinct_ids) > 1:
            return PlaymatchResult(
                ...
                found=False,
                needs_manual_review=True,
                manual_review_reason="conflicting Playmatch identities for same title",
                match_method=PlaymatchMatchMethod.MANUAL_REVIEW,
        ...
        return PlaymatchResult(
            ...
            found=True,
            category=best.category,
            match_method=PlaymatchMatchMethod.FUZZY_TITLE,
